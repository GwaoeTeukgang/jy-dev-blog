...
import headerStyle, {mobileNavStyle} from '@/app/_component/header/header.style';
...
export default function Header() {
    ...
    const {mobileNav, menuIcon, mobileNavItem} = mobileNavStyle({state: navOpen ? 'open' : 'close'});
                            ...
                            <ul>
                                {
                                    HEADER_NAVIGATION.map(({path, name}) =>
                                        <li key={name} style={{marginBottom: '4px'}}>
                                            <Link className={mobileNavItem()} data-active={segment === path} href={`/${path}`}>{name}</Link>
                                        </li>)
                                }
                            </ul>
                        </nav>
