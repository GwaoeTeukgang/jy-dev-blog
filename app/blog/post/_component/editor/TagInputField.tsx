...
export default function TagInputField({onChange, disabled, selectedTags}: TextInputProps) {
      ...
      <div className={'mb-4'} >
          ...
          <div className={style.container({opened: isOpen})}>
              <ul className={style.list()}>
                  {tags.map((it) =>
                      <li className={style.listItem({selected: isSelected(it.id)})}
                          onClick={(e) => onAddTag(it)}
                          key={it.id}>{it.tagLabel}</li>)}
              </ul>
              <button onClick={() => setIsOpen(false)}
                      type={'button'}
                      className={style.closeButton()}>닫기</button>
          </div>
          <ul className={'mt-4 flex flex-wrap gap-1'}>
          {selectedTag.map(it =>
                  <li key={it.id}
                      onClick={() => onRemoveTag(it)}>
